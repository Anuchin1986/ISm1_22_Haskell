...
longest (x : xs) = longest' xs x
 where
  compareLists x y | length x == length y = if head x > head y then y else x
                   | length x > length y  = x
                   | otherwise            = y
  longest' xs acc = foldl compareLists acc xs
...
incrementKey _ [] = []
incrementKey k (x@(key, v) : xs) =
  if k == key then (key, v + 1) : incrementKey k xs else x : incrementKey k xs
...
average xs = addValues xs 0 / fromIntegral (length xs)
 where
  addValues []       acc = acc
  addValues (x : xs) acc = addValues xs (acc + x)
...
winner scores player1 player2 =
  let score1 = Map.findWithDefault 0 player1 scores
      ...
      winner' score1 score2 player1 player2 | score1 == score2 = player1
                                            | score1 > score2  = player1
                                            | otherwise        = player2
  in  winner' score1 score2 player1 player2
...
findMax :: (Ix i, Ord t) => [(i, t)] -> t -> i -> i
findMax [] _ i = i
findMax ((i, v) : xs) acc y =
  if acc >= v then findMax xs v y else findMax xs v i

maxIndex :: (Ix i, Ord a) => Array i a -> i
maxIndex arr = findMax (Data.Array.assocs arr)
                       (head . Data.Array.elems $ arr)
                       (head . Data.Array.indices $ arr)
