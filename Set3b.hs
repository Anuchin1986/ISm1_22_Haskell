...
sumsHelper _ 0 _ = []
sumsHelper start count acc = (acc + start) : sumsHelper (start + 1) (count - 1) (acc + start)
...
sumsOf [] = []  
sumsOf [x] = [x]  
sumsOf (x:xs) = x : mapAccumSums x xs
  where
    mapAccumSums _ [] = []
    mapAccumSums prevSum (y:ys) =
      let currentSum = prevSum + y
      in currentSum : mapAccumSums currentSum ys
