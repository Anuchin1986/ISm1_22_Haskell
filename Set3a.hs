...
firstHalf str = take (length str `div` 2 + length str `mod` 2) str
...
whileRight check val =
  case check val of
    Left result -> result
    Right updatedVal -> whileRight check updatedVal
...
multiApp f xs y = f $ multiApp' xs y
  where
    multiApp' [] _ = []
...
interpreter commands = interpret commands 0 0
  where
    interpret [] _ _ = []
    interpret ("up":rest) x y = interpret rest x (y + 1)
    interpret ("down":rest) x y = interpret rest x (y - 1)
    interpret ("left":rest) x y = interpret rest (x - 1) y
    interpret ("right":rest) x y = interpret rest (x + 1) y
    interpret ("printX":rest) x y = show x : interpret rest x y
    interpret ("printY":rest) x y = show y : interpret rest x y
